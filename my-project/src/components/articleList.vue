...
<script type="text/ecmascript-6">
    export default {
      beforeCreate(){
        //跳转到list页面之前将type值存入store
        if(JSON.stringify(this.$route.params) != '{}')
        {
          this.$store.commit('setListArticleType',this.$route.params.type)
          this.type = this.$store.state.listArticleType
        }

        //跳转到list页面之前通过之前存入的type值获取该列表的新闻
        const params = {"type":this.$store.state.listArticleType,"curPage":1};
        this.$axios.post("/setArticleList",params).then((result)=>{
      ...
      methods:{
        ...
        getListData(doPage){
          ...
          const params = {"type":this.$store.state.listArticleType,"curPage":this.curPage}
